...
positions_A, positions_B, positions_C, positions_D, positions_E, positions_F = list(), list(), list(), list(), list(), list()
...
new_chain_F = new_topology.addChain(id="F")

# Specify the starting residue ids for each chain
d_current_start = {"C": 18, "E": 0, "A": 332, "B": 729, "D": 739}
...
for res in old_topology.residues():  
    ...
    if res.name not in ['HOH', 'Na+', 'Cl-']:
        ...
        new_res = new_topology.addResidue(res.name, new_chain_F, id=res.id, insertionCode=res.insertionCode)
        for atom in res.atoms():
            new_atom = new_topology.addAtom(atom.name, atom.element, new_res)
            d_old_to_new[atom] = new_atom
            positions_F.append(old_positions[atom.index])
...
new_positions = positions_A + positions_B + positions_C + positions_D + positions_E + positions_F
